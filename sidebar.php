<div class="right">
	...
	<?php  if (textReturn('is_new_list',0)) { ?>
	...
	<?php  if (textReturn('is_round_list',0)) { ?>
			...
			<div class="right_round">
				<?php
						$i=0;
							...
							$query_posts->query($args);
							while( $query_posts->have_posts() ) { $query_posts->the_post(); ?>
				<a class="round_list" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
	...
	<?php  }  ?>
	<?php  if (textReturn('is_link',0)) { ?>
	<div class="rihgt_list">
		<h3>友情链接</h3>
		<p> <?php 
					if(!textReturn('link_desc',0))    echo '请前往国瑞后台系统设置,<a href="/wp-admin/themes.php?page=wp-theme-options.php" target="_blank">点击跳转</a>';
					else                              textReturn('link_desc');  
